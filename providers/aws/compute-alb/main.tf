...
# CloudWatch Alarm for ALB Request Count Per Target (High)
resource "aws_cloudwatch_metric_alarm" "cpu_utilization_high" {
  alarm_name          = "${local.prefix}-compute-requests-high"
  ...
  period              = "60"
  statistic           = "Sum"
  threshold           = var.scaling_request_count_per_target
  alarm_description   = "Triggers scale-up when ALB requests per target exceed threshold"
...
# CloudWatch Alarm for ALB Request Count Per Target (Low)
resource "aws_cloudwatch_metric_alarm" "cpu_utilization_low" {
  alarm_name          = "${local.prefix}-compute-requests-low"
  ...
  period              = "60"
  statistic           = "Sum"
  threshold           = var.scaling_down_request_count_per_target
  alarm_description   = "Triggers scale-down when ALB requests per target drop below threshold"
